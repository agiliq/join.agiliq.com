...
def generate_token():
    return "".join([random.choice(CLIENT_PARAMS_SPACE) for ii in range(0, 50)]) 
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.POST.get("authorized") == "Reject":
                return HttpResponseRedirect("%s?%s" % (application.redirect_uri,
                                                       urllib.urlencode({"error": "access_denied",
                                                                         "error_description": "User has rejected the authorization"})))
            ...
            AuthorizationCode.objects.filter(application=application, user_profile=user_profile).delete()
            auth_code = AuthorizationCode.objects.create(application=application,
...
def oauth_access_token(request):
    ...
    access_token = AccessToken.objects.create(user_profile=authorization.user_profile,
    ...
    return HttpResponse(json.dumps(access_params), content_type="application/json")
